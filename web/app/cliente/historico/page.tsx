...
import { isAppointmentUpcoming, parseAppointmentWallClock } from '@/lib/timeUtils'
...
export default function HistoricoPage() {
  ...
  const [hiddenIdsReady, setHiddenIdsReady] = useState(false)
  
  // Carrega IDs ocultados do localStorage ao montar (fallback otimista)
  useEffect(() => {
    if (!user) {
      setHiddenIdsReady(false)
      return
    }
    const stored = localStorage.getItem(`hiddenHistory_${user.id}`)
    if (stored) {
      try {
        setHiddenIds(new Set(JSON.parse(stored) as string[]))
      } catch (e) {
    ...
    setHiddenIdsReady(true)
  }, [user])
  
  // Persiste IDs ocultados no localStorage (só após hidratar, para não apagar dados)
  useEffect(() => {
    if (!user || !hiddenIdsReady) return
    localStorage.setItem(`hiddenHistory_${user.id}`, JSON.stringify(Array.from(hiddenIds)))
  }, [hiddenIds, user, hiddenIdsReady])

  // Filtrar histórico: concluídos/cancelados OU cuja hora de parede já passou
  let pastAppointments = appointments.filter(apt => {
    if (hiddenIds.has(apt.id)) return false
    if (['COMPLETED', 'CANCELED', 'NO_SHOW'].includes(apt.status)) return true
    // PENDING/CONFIRMED cujo horário de parede já passou → histórico
    if (['PENDING', 'CONFIRMED'].includes(apt.status) && !isAppointmentUpcoming(apt.startTime)) return true
    return false
  })
  ...
  if (startDateFilter) {
    ...
    pastAppointments = pastAppointments.filter(apt => 
      parseAppointmentWallClock(apt.startTime) >= startDate
    )
  }
  ...
  if (endDateFilter) {
    ...
    pastAppointments = pastAppointments.filter(apt => 
      parseAppointmentWallClock(apt.startTime) <= endDate
    )
  }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      // Adiciona ao Set de IDs ocultados (fallback local + UI imediata)
  ...
  const handleClearAll = async () => {
    ...
    const idsToHide = pastAppointments.map(apt => apt.id)
    const fallbackCount = idsToHide.length
    setClearingAll(true)
    try {
      const result = await api.clearHistory(user.id)
      
      // Merge com IDs já ocultados (não substitui o Set inteiro)
      setHiddenIds(prev => {
      ...
      const count = result?.count ?? fallbackCount
      toast.success(`${count} tratamento(s) removido(s) do histórico`)
