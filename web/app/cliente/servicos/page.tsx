                      ...
                      <div className="text-right ml-4">
                        {freeVoucher ? (
                          <div>
                            ...
                            <div className="text-xs text-gray-400 line-through">{formatBRL(service.price)}</div>
                          </div>
                        ) : includedInPlan ? (
                          <div>
                            <div className="text-sm font-bold text-green-600">Incluso</div>
                            <div className="text-xs text-gray-500 mt-0.5">{formatBRL(service.price)}</div>
                          </div>
                        ) : hasDiscount ? (
                          <div>
                            <div className="text-lg font-bold text-pink-600">{formatBRL(pricing.final)}</div>
                            <div className="text-xs text-gray-400 line-through">{formatBRL(pricing.original)}</div>
                          </div>
                        ) : (
                          <div className="text-lg font-bold text-pink-600">{formatBRL(service.price)}</div>
                        )}
                      </div>
                    </div>
                  </button>
